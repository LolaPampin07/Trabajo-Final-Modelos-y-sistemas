...
t_span_lin = linspace(0, 50, 2000); %para sistema lineal necesito tiempo más corto
...
[t2a, X2a] = ode45(f_lin2, t_span_lin, X0_lin2_a);
[t2b, X2b] = ode45(f_lin2, t_span_lin, X0_lin2_b);
[t2c, X2c] = ode45(f_lin2, t_span_lin, X0_lin2_c);

% Figura 5: Trayectoria desde xi_0
figure(5); 
plot3(X2a(:, 1), X2a(:, 2), X2a(:, 3), 'g-', 'LineWidth', 1.5); hold on;
% Marco el origen (xeq2)
plot3(0, 0, 0, 'k*', 'MarkerSize', 15, 'LineWidth', 2); 
title('Diagrama de Fase (Xeq2) - Desde Cond. Inicial xi_0');
xlabel('x'); ylabel('y'); zlabel('z');
legend('Trayectoria desde xi_0', 'Punto de Equilibrio Xeq2'); 
grid on; hold off;
axis equal;

% Figura 6: Trayectoria desde xi_1
figure(6); 
plot3(X2b(:, 1), X2b(:, 2), X2b(:, 3), 'b--', 'LineWidth', 1.5); hold on;
plot3(0, 0, 0, 'k*', 'MarkerSize', 15, 'LineWidth', 2); 
title('Diagrama de Fase (Xeq2) - Desde Cond. Inicial xi_1');
xlabel('x'); ylabel('y'); zlabel('z');
legend('Trayectoria desde xi_1', 'Punto de Equilibrio Xeq2');
grid on; hold off;
axis equal;

% Figura 7: Trayectoria desde xi_2
figure(7);
plot3(X2c(:, 1), X2c(:, 2), X2c(:, 3), 'r:', 'LineWidth', 1.5); hold on;
plot3(0, 0, 0, 'k*', 'MarkerSize', 15, 'LineWidth', 2); 
title('Diagrama de Fase (Xeq2) - Desde Cond. Inicial xi_2');
xlabel('x'); ylabel('y'); zlabel('z');
legend('Trayectoria desde xi_2', 'Punto de Equilibrio Xeq2'); % 
grid on; hold off;
axis equal;
...
%Respuesta al Impulso y al Escalón - Xeq1

figure(10); 

subplot(2, 1, 1);
impulse(G1_tf);
title('Respuesta al Impulso - Sistema Linealizado en Xeq1');
grid on;

subplot(2, 1, 2); 
step(G1_tf);
title('Respuesta al Escalón - Sistema Linealizado en Xeq1');
grid on;
...
subplot(2, 1, 1);
impulse(G2_tf);
title('Respuesta al Impulso - Sistema Linealizado en Xeq2');
grid on;

subplot(2, 1, 2); 
step(G2_tf);
title('Respuesta al Escalón - Sistema Linealizado en Xeq2');
grid on;
